...
layout(push_constant) uniform PreFilterPushConstant
{
    uint hdriIndex;
    float roughness;
};
...
void main()
{
    vec3 direction = MapDirection(texCoords, gl_ViewIndex);
    ...
    for (uint i = 0; i < SAMPLE_COUNT; ++i)
    {
        ...
        if (NoL > 0.0)
        {
            prefilteredColor += clamp(texture(samplers[hdriIndex], SampleSphericalMap(L)).rgb, 0, RADIANCE_CLAMP_VAL) * NoL;
            totalWeight += NoL;
        }
    }
